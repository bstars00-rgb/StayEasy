...
function userFrom(row) { return row && { id: row.id, provider: row.provider, name: row.name, email: row.email, picture: row.picture_url || '', role: row.role || 'operator', createdAt: row.created_at, updatedAt: row.updated_at } }
...
export class OhmySelectStore {
  ...
  async migrate() {
    await this.db.exec(`
      CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, provider TEXT NOT NULL, provider_subject TEXT NOT NULL UNIQUE, name TEXT NOT NULL, email TEXT NOT NULL UNIQUE, picture_url TEXT, role TEXT DEFAULT 'operator', created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
    ...
    `)
    await this.db.run("ALTER TABLE users ADD COLUMN role TEXT DEFAULT 'operator'").catch(() => {})
  ...
  async createVoucher(membershipId, input, actor) {
    ...
    await this.db.run('INSERT INTO voucher_templates (id,membership_id,template_id,category,title,description,quantity,valid_until,city_id,hotels,transferable,note) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',[id,membershipId,input.templateId,input.category||'other',input.title,input.description||'',Number(input.quantity||1),input.validUntil||'2026-12-31',input.city||null,stringify(input.hotels||[]),input.transferable?1:0,input.note||''])
  ...
  async updateVoucher(templateId, input, actor) {
    ...
    await this.db.run('UPDATE voucher_templates SET category=?,title=?,description=?,quantity=?,valid_until=?,city_id=?,hotels=?,transferable=?,note=? WHERE template_id=?',[next.category,next.title,next.description||'',Number(next.quantity||0),next.validUntil,next.city||null,stringify(next.hotels||[]),next.transferable?1:0,next.note||'',templateId])
